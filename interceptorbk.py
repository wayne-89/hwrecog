...
def doIntercept(filename):
    ...
    for j in range(0, detectRowNum):
        for i in range(0, detectColNum):
            ...
            if len(contoursInner) > 0:
                ...
                node = boxTable[j][i] = Node(x + boundRectInner[0],
                                             y + boundRectInner[1],
                                             boundRectInner[2],
                                             boundRectInner[3])
                # print(node.x, node.y, node.w, node.h)
                img_dc = cv.rectangle(img_dc, (node.x, node.y),
    ...
    cv.imwrite(getFilePath(filename, 'result.png'), img_dc)
    cv.imwrite(getFilePath(filename, 'eng.' + filename + '.exp1.tif'), img_src)
    # ################################

    # 输出box文件
    writeBoxFile(template, boxTable, filename, img.shape[0])
    return True
    # cv.waitKey(0)
